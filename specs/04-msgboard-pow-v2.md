...
>
> **This is the algorithm reth implements.** It replaced the construction in
> `specs/02-msgboard.md` under the same version byte — `version = 1` — so a
> message mined under the old rules no longer verifies, and there is no
> dual-accept path. See `docs/msgboard-parity-gaps.md` §17 for the analysis and
> §20 for what the spec still leaves to the implementer.
>
> Two things here have no upstream counterpart. The `TestPoWGoldenVector` cited
> under *Reference Implementations* does not exist in erigon-pulse at
> `48cdb29e35`; ours is in `crates/net/msgboard-types/src/pow.rs` (mod
> `golden_vector`), generated by `scripts/msgboard-pow-vector.js`. And the spec
> gives the poster's rules but never the receiver's, which leaves `D` unbounded
> below — see §20.
>
